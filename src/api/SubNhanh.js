...
const URL = {
    DOMAIN: "https://subnhanh.net/",
    URL_GET_LINK_STREAM: "https://api-v3.bongngo.info/ajax/episodes",
    SEARCH: "https://subnhanh.net/search?query=",
    HEADERS: {
        'Origin': '',
...
export default class SubNhanh {
    ...
    getLinkStreamFromEpisodeData = async (fid, epId, urlAjax) => {
        try {
            ...
            for (let index = 0; index < listServer.length; index++) {
                ...
                try{
                    let res = await axios.post(urlRequest, form_data,
                        { headers });
                    ...
                    if (id) {
                        ...
                        let hostname = url_parts.hostname
                        let urlPlaylist = `https://${hostname}/hls/${id}/playlist.m3u8`
                        server.urlPlaylistHls = urlPlaylist
                        let sourcePlaylist = []
                        let quality = {};
    ...
    getDetailMovie = async function (urlDetail) {
        try {
            ...
            let response = await axios.get(urlDetail, { httpsAgent: agent })
    ...
    getListServerAndEpisode = async function (urlOther) {
        try {
            ...
            let response = await axios.get(urlOther, { httpsAgent: agent })
            const $ = cheerio.load(response.data)
            let html = $.html()
    ...
    getItemMove = function ($, htmlMovie) {
        ...
        var urlPhoto = URL.DOMAIN + variable.match(/url\(["']?([^"']*)["']?\)/)[1].substring(1);
        return {
            title,
            status,
            urlDetail,
    ...
    */
    ...
    getQualityHls = async function (urlPlaylist,domain) {
        let response = await axios.get(urlPlaylist, { httpsAgent: agent })
...
export async function getHomeMovies() {
    try {
        ...
        //   axios.get('https://something.com/foo', { httpsAgent: agent });
        let response = await axios.get(URL.DOMAIN, { httpsAgent: agent })
        const $ = cheerio.load(response.data);
        const listCollection = $(".latest")
        let listCollectionMovie = []
        for (let i = 0; i < listCollection.length; i++) {
            let listMovieInCollection = $(listCollection[i]).find(".los").find('.box')
            let collectionName = $(listCollection[i]).find(".more > h2").text()
            let urlSeeMore = $(listCollection[i]).find(".more > a").attr("href")
            let listMovie = []
            for (let index = 0; index < listMovieInCollection.length; index++) {
                let movieInHtml = listMovieInCollection[index];
